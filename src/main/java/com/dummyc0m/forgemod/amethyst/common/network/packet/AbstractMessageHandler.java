package com.dummyc0m.forgemod.amethyst.common.network.packet;
    ...
    */
    ...
    public IMessage onMessage(T message, MessageContext ctx) {
        ...
        if (ctx.side.isClient()) {
            ...
            return handleClientMessage(AmethystMod.proxy.getPlayerEntity(ctx), message, ctx);
        } else {
            // server side proxy will return the server side EntityPlayer
            return handleServerMessage(AmethystMod.proxy.getPlayerEntity(ctx), message, ctx);
        }
    }
}